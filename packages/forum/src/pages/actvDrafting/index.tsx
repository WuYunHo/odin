...
import { AlignLeftOutlined, CheckOutlined, CloseOutlined, DeleteOutlined, EditOutlined, EnterOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
...
const AccessPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios.post('/api/forumapi/findactvDrafting', {
      // leaderID: user.id
    }).then(res=>{
  ...
  const handlePubing = (item: any) => {
      ...
      //request更新状态
      axios.post('/api/forumapi/pubingActivity', {
        actvid: item.actvid
      }).then(res=>{
        setchange(Math.random())
      })
